...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $.ajax({
            url : '${pageContext.request.contextPath}/sys/sfzshow.json',
            type : 'GET',
            data : null,
            dataType : 'json',
        ...
        table.on('tool(user)',function(obj) {
            ...
            if (obj.event === 'del') {
                layer.confirm('真的删除行么', function(index) {
                    var a=data.id;
                    window.location.href = '${pageContext.request.contextPath}/sys/sfzdel.json?id='+data.id;
                    layer.close(index);
                ...
                $(function () {
                    ...
                    $.ajax({
                        url: '${pageContext.request.contextPath}/sys/sfzid.json',
                        type: 'GET',
                        data: {"id": a},
                        dataType: 'json',
                        success: function (result) {
                            ...
                            layer.confirm('<div style="padding: 50px 30px;">' + '<b>您正在进行修改操作</b><hr><br><form action="${pageContext.request.contextPath}/sys/sfzupdate.json" method="get" id="Sub">类型名称:<input type="text" name="configTypeName" id="name"><br><br>是否启用:<select  name="isStart" id="index"><option  value="1">启用</option><option   value="2">不启用</option></select><br> <br><input type="hidden" id="sa" name="id"></form>' + '</div>', {
                                btn: ['修改', '取消'] //可以无限个按钮
    ...
    function test(){
        layer.confirm('<div style="padding: 50px 30px;">' + '<b>您正在进行修改操作</b><hr><br><form action="${pageContext.request.contextPath}/sys/sfzadd.json" method="get" id="Sub">类型名称:<input type="text" name="configTypeName" id="name"><br><br>是否启用:<select  name="isStart" id="index"><option  value="1">启用</option><option   value="2">不启用</option></select><br> <br><input type="hidden" id="sa" name="configType" value="6"></form>' + '</div>', {
            btn: ['添加', '取消'] //可以无限个按钮
            ,yes: function(index, layero){
                $("#Sub").submit();
            }
