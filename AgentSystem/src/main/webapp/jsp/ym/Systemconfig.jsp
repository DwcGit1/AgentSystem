    ...
    <head>
        ...
        <script>
            ...
            function updateconfig(thist,id){
                ...
                var isStart=$(thist).parent().parent().children().eq(2).text();
                /*if(typeof userPassword=="number"){
                    userPassword+='';
                }*/
                console.log($(thist)+'-----------------')
                layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行修改操作</b><hr><br>' +
                    '<form action="${pageContext.request.contextPath}/systemconfig/updateconfig?id='+id+'" method="post" id="Sub"><table  align="center" style="border-collapse:separate; border-spacing:0px 10px;">'
                    +'<tr align="center"><td align="center">角色名称:<input type="text" name="configTypeName" id="configTypeName" style="width:150px;" value='+configTypeName+'></td></tr>'+
            ...
            function add(){
                layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行添加操作</b><hr><br>' +
                    '<form action="${pageContext.request.contextPath}/systemconfig/addconfig" method="post" id="Sub"><table  align="center" style="border-collapse:separate; border-spacing:0px 10px;">'
                    +'<tr align="center"><td align="center">配置类型:'+'<input type="text" name="configTypeName" id="configTypeName" style="width:150px;">'+'</td></tr>'
            ...
            function delet(id){
                ...
                if (confirm(msg)==true){
                    ...
                    window.location.href="${pageContext.request.contextPath}/systemconfig/deleteconfigs/?id="+id;
                    //你也可以在这里做其他的操作
                }else{
                    return false;
                }
...
<table style="width:1300px;" class="layui-table" align="center">
    <tr align="center">
        <td align="center"><h2><span style="text-align: center">财务类型</span></h2></td>
    </tr>
</table>
...
<table class="layui-table" style="width: 1300px;text-align: center">
    ...
    <tbody align="center">
    <c:forEach items="${systemConfigs}" var="systemConfigsInfo" varStatus="status">
        <tr align="center" style="text-align: center">
            <th align="center" width="17%" style="text-align: center">${systemConfigsInfo.id}</th>
            <th align="center" width="17%" style="text-align: center">${systemConfigsInfo.configTypeName}</th>

            <c:if test="${systemConfigsInfo.isStart==1}"><th width="17%" style="text-align: center">启用</th></c:if>
            <c:if test="${systemConfigsInfo.isStart==0}"><th width="17%" style="text-align: center">不启用</th></c:if>
            <th width="20%" align="center" style="text-align: center">
                ...
                <button type="button" class="layui-btn layui-btn-sm" onclick="updateconfig(this,${systemConfigsInfo.id})">
                    <i class="layui-icon">&#xe642;</i>
                </button>

                    <%-- <a href="${pageContext.request.contextPath}/role/deleteRoles?id=${roleInfo.id}">--%>
                <button type="button" class="layui-btn layui-btn-sm" onclick="delet(${systemConfigsInfo.id})">
                    <i class="layui-icon">&#xe640;
                    </i>
                </button>
                    <%--</a>--%>
