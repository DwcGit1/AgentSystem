...
<head>
    ...
    <script>
        ...
        function updateRole(thist,id){
            var roleName=$(thist).parent().parent().children().eq(0).text();
            ...
            layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行修改操作</b><hr><br>' +
                '<form action="${pageContext.request.contextPath}/role/updateRoles?id='+id+'" method="post" id="Sub"><table  align="center" style="border-collapse:separate; border-spacing:0px 10px;">'
                +'<tr align="center"><td align="center">角色名称:<input type="text" name="roleName" id="roleName" style="width:150px;" value='+roleName+'></td></tr>'+
        ...
        function add(){
            layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行添加操作</b><hr><br>' +
                '<form action="${pageContext.request.contextPath}/role/addRoles" method="post" id="Sub"><table  align="center" style="border-collapse:separate; border-spacing:0px 10px;">'
                +'<tr align="center"><td align="center">用户角色:'+'<input type="text" name="roleName" id="roleName" style="width:150px;">'+'</td></tr>'
                ...
                '<option  value="1">启用' + '</option><option value="2">不启用</option></select></td><tr>'+
                '<input type="hidden" id="sa" name="id"></table></form>'+'</div>',{
                btn: ['保存', '取消']
                ,yes: function(index, layero){
                    $("#Sub").submit();
        ...
        function delet(id){
            ...
            if (confirm(msg)==true){
                ...
                window.location.href="${pageContext.request.contextPath}/role/deleteRoles/?id="+id;
                //你也可以在这里做其他的操作
            }else{
                return false;
            }
...
<table style="width:1300px;" class="layui-table" align="center">
    <tr align="center">
        <td align="center"><h2><span style="text-align: center">角色管理</span></h2></td>
    </tr>
</table>
...
<table class="layui-table" style="width: 1300px;text-align: center">
    ...
    <c:forEach items="${role}" var="roleInfo" varStatus="status">
        <tr align="center" style="text-align: center">
            <th align="center" width="17%" style="text-align: center">${roleInfo.roleName}</th>
            <th align="center" width="17%" style="text-align: center">
                    <span><fmt:formatDate value="${roleInfo.creationTime}" type="both" />
            </th>
            <c:if test="${roleInfo.isStart==1}"><th width="17%" style="text-align: center">是</th></c:if>
            <c:if test="${roleInfo.isStart==2}"><th width="17%" style="text-align: center">否</th></c:if>
            <th width="20%" align="center" style="text-align: center">
                ...
                <button type="button" class="layui-btn layui-btn-sm" onclick="updateRole(this,${roleInfo.id})">
                    <i class="layui-icon">&#xe642;</i>
                </button>

               <%-- <a href="${pageContext.request.contextPath}/role/deleteRoles?id=${roleInfo.id}">--%>
                    <button type="button" class="layui-btn layui-btn-sm" onclick="delet(${roleInfo.id})">
                        <i class="layui-icon">&#xe640;
                        </i>
                    </button>
                <%--</a>--%>
